  ...
  return (
    <Link
      ...
      className={cn(
        ...
        isActive && 'border-b-fuchsia-700 hover:border-b-fuchsia-700 text-fuchsia-700 hover:text-fuchsia-700'
      )}
    >
      <div className="flex flex-col items-center px-2 py-2 my-1 rounded-lg">
        {icon}
        <div className={cn("text-xs font-light transition-all", isActive && "font-medium")}>{name}</div>
      </div>
    </Link>
  );
}
  ...
  return (
    <nav className="w-full bg-white dark:bg-black shadow-sm border-b-gray-100">
      <div className="container mx-auto flex items-center gap-2">
        ...
        <Item
          href="/hotels"
          icon={<BedDoubleIcon className="w-6 h-6 mb-2" />}
          name="Hoteles"
        />
        <Item
          href="/restaurants"
